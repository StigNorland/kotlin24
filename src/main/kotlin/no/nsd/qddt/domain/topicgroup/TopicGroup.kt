...
class TopicGroup(
  ...
  override fun fillDoc(pdfReport:PdfReport, counter:String) {
    ...
    if (topicQuestionItems.size > 0)
    {
      ...
      {
        pdfReport.addHeader2(item.element!!.name, String.format("Version %s", item.element!!.version))
        pdfReport.addParagraph(item.element!!.question)
        if (item.element!!.responseDomainRef.element != null)
        item.element!!.responseDomainRef.element!!.fillDoc(pdfReport, "")
      }
    }
